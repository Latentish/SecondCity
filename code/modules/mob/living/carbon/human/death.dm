...
/mob/living/carbon/human/death(gibbed)
	...
	if(SSticker.HasRoundStarted())
		...
		log_message("has died (BRUTE: [src.get_brute_loss()], BURN: [src.get_fire_loss()], TOX: [src.get_tox_loss()], OXY: [src.get_oxy_loss()], AGG: [src.getAggLoss()]", LOG_ATTACK) // DARKPACK EDIT CHANGE - AGGRAVATED_DAMAGE
		if(key) // Prevents log spamming of keyless mob deaths (like xenobio monkeys)
			investigate_log("has died at [loc_name(src)].<br>\
				BRUTE: [src.get_brute_loss()] BURN: [src.get_fire_loss()] TOX: [src.get_tox_loss()] OXY: [src.get_oxy_loss()] STAM: [src.get_stamina_loss()] AGG: [src.getAggLoss()]<br>\
