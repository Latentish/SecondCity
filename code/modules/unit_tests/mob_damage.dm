...
/datum/unit_test/mob_damage/animal/verify_damage(mob/living/carbon/testing_mob, amount, expected, included_types = ALL)
	...
	if(included_types & BRUTELOSS)
		TEST_ASSERT_EQUAL(round(testing_mob.get_brute_loss(), 1), expected || amount * 5, \
			"[testing_mob] should have [expected || amount * 5] brute damage, instead they have [testing_mob.get_brute_loss()]!") // DARKPACK EDIT CHANGE - AGGRAVATED_DAMAGE
	if(included_types & FIRELOSS)
